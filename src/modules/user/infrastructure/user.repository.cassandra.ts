...
export class UserRepositoryCassandra implements UserRepository {
  ...
  async create(props: {
    ...
    await this.client.execute(
      ...
      [props.email.toLowerCase(), id, props.name, props.passwordHash, props.birthDate, props.role, now, now, isActive],
      { prepare: true }
    );
    await this.client.execute(
      "INSERT INTO users_by_id (user_id, email, name, password_hash, birth_date, role, created_at, updated_at, is_active) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [id, props.email.toLowerCase(), props.name, props.passwordHash, props.birthDate, props.role, now, now, isActive],
      { prepare: true }
    );
    await this.client.execute(
  ...
  async findAll(): Promise<UserEntity[]> {
    const res = await this.client.execute("SELECT user_id, email, name, password_hash, birth_date, role, created_at, updated_at, is_active FROM users_by_id");
  ...
  async save(user: UserEntity): Promise<void> {
    ...
    await this.client.execute(
      ...
      [user.email, user.name, user.passwordHash, user.birthDate, user.role, now, user.isActive, user.id],
      { prepare: true }
    );
    await this.client.execute(
      "UPDATE users_by_email SET user_id = ?, name = ?, password_hash = ?, birth_date = ?, role = ?, updated_at = ?, is_active = ? WHERE email = ?",
      [user.id, user.name, user.passwordHash, user.birthDate, user.role, now, user.isActive, user.email],
      { prepare: true }
    );
  }
